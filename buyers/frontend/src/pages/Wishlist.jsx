        ...
        <div className="max-w-[1440px] mx-auto px-4 md:px-8 relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-8">
          ...
          {wishlistItems.length > 0 && (
            <FadeUp delay={0.1}>
              <button 
                onClick={handleClear}
                className="group flex items-center gap-3 px-6 py-3 bg-base-100 hover:bg-error/10 border border-base-300 hover:border-error/30 rounded-2xl transition-all duration-300"
              >
      ...
      <div className="max-w-[1440px] mx-auto px-4 md:px-8 py-12 md:py-20">
        {loading ? (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
            {[1, 2, 3, 4, 5].map(i => (
              <div key={i} className="aspect-[3/4] bg-base-100 animate-pulse rounded-[2rem] border border-base-200" />
            ))}
          </div>
        ) : wishlistItems.length === 0 ? (
          <FadeUp>
              ...
              {wishlistItems.map((product, i) => (
                ...
                >
                  ...
                  <button 
                    ...
                    className="absolute top-4 right-4 z-40 w-10 h-10 bg-black/80  rounded-xl flex items-center justify-center text-white opacity-0 group-hover:opacity-100 hover:bg-error transition-all duration-300 shadow-xl border border-white/10"
                  ...
                  <Link 
                    ...
                    className="absolute bottom-20 left-4 right-4 z-20 py-3 bg-white/10  border border-white/20 rounded-xl text-xs font-black text-white uppercase tracking-widest text-center opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-500 hover:bg-primary hover:text-primary-content"
                  >
                    View Details
                  </Link>
                </motion.div>
